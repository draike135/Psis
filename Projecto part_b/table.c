...
void* keyboard_listener(void* arg) {
  termina *data = (termina *)arg;
  message_display m_disp;
  remote_char_t m;
  void *context = data->context; // Use the same context
  void *requester = zmq_socket(context, ZMQ_REQ);
  zmq_connect(requester, "tcp://localhost:5556");

  void *term_socket = zmq_socket(context, ZMQ_REQ);
  zmq_connect(term_socket,  "tcp://localhost:6025");


    //printf("Press 'Q' or 'q' to exit...\n");
    while (1) {
        char key = getch();  // Waits for a key press
        if (key == 'Q' || key == 'q') {

            printf("Terminating program...\n");
            m_disp.copy_message.msg_type=9;
            m.msg_type=9;
            zmq_send(requester, "TERMINATE", 9, 0);//terminate astronaut clints
            zmq_send(data->pub, &m_disp, sizeof(message_display), 0);// terminate displays
            zmq_send(term_socket, &m, sizeof(m), 0);// thermiante comunication thread
            zmq_close(requester);
            zmq_close(term_socket);

            break;
            //exit(0);  // Exit the program when 'Q' or 'q' is pressed
        }
    }
    printf("saiii\n");
    return NULL;
